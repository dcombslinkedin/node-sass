...
module.exports.render = function(opts, cb) {
  let h = heimdall.start('node-sass:render');

  try {
    var options = getOptions(opts, cb);

    console.log("------- Calling node-sass.render at " + Date.now() + ", some options = ");
    console.log("-------   file: " + options.file);
    console.log("-------   outFile: " + options.outFile);
    console.log("-------   includePaths: " + options.includePaths);

    let t1 = 0;

    // options.error and options.success are for libsass binding
    options.error = function(err) {
      console.log("-------  options.error at " + Date.now());
    ...
    options.success = function() {
      ...
      console.log("-------  options.success at " + t2 + " (total " + (t2 - t1) + " ms)");
      ...
      if (cb) {
        console.log("-------  options.callback at " + Date.now());
        ...
        importer.forEach(function(subject, index) {
          options.importer[index] = function(file, prev, bridge) {
            function done(result) {
              console.log("### importer done for file " + file + " at " + end + " (total = " + (end - start) + " ms)");
              bridge.success(result === module.exports.NULL ? null : result);
            }

            start = Date.now();
            console.log("### Calling importer for file " + file + " at " + start);
    ...
    if (options.data) {
      console.log("-------  calling binding.render at " + t1);
      binding.render(options);
    } else if (options.file) {
      console.log("-------  calling binding.renderFile (" + options.file + ") at " + t1);
...
module.exports.renderSync = function(opts) {
  let h = heimdall.start('node-sass:renderSync');

  try {
    var options = getOptions(opts);
    var importer = options.importer;

    console.log("##### Calling node-sass.renderSync, options = " + JSON.stringify(options));
